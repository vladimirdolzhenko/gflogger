 ...
 */
public class DFormattedLoggerExample extends AbstractLoggerExample {
	...
	protected void logDebugTestMessage(int i) {
		log.debug("test%s").withLast(i);
	}

	@Override
	protected void logMessage(String msg, int j) {
		log.info("%s%s").with(msg).withLast(j);
	}

	@Override
	protected String fileAppenderFileName() {
		return getStringProperty("org.gflogger.filename", "./logs/dgflogger_f.log");
	}

	public static void main(final String[] args) throws Throwable {
		if (args.length > 2) System.in.read();
		final DFormattedLoggerExample loggerExample = new DFormattedLoggerExample();
